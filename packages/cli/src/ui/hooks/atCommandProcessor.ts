...
import type { AnyToolInvocation, Config } from '@unipath/unipath-cli-core';
import {
  getErrorMessage,
  isNodeError,
  unescapePath,
...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const unipathIgnored =
      respectFileIgnore.respectUnipathIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectUnipathIgnore: true,
      });

    if (gitIgnored || unipathIgnored) {
      const reason =
        gitIgnored && unipathIgnored ? 'both' : gitIgnored ? 'git' : 'unipath';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and unipath'
          : reason === 'git'
            ? 'git-ignored'
            : 'unipath-ignored';
  ...
  const totalIgnored =
    ...
    ignoredByReason['unipath'].length +
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['unipath'].length) {
      messages.push(`Unipath-ignored: ${ignoredByReason['unipath'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_unipath_ignore: respectFileIgnore.respectUnipathIgnore,
    },
