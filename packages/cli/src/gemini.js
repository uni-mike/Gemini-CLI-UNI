...
const InitializingComponent = ({ initialTotal }) => {
    const [total, setTotal] = useState(initialTotal);
    const [connected, setConnected] = useState(0);
    useEffect(() => {
        const onStart = ({ count }) => setTotal(count);
        const onChange = () => {
    ...
    return (_jsx(Box, { children: _jsxs(Text, { children: [_jsx(Spinner, {}), " ", message] }) }));
};
...
export async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd()) {
    const version = await getCliVersion();
    // Detect and enable Kitty keyboard protocol once at startup
    await detectAndEnableKittyProtocol();
    setWindowTitle(basename(workspaceRoot), settings);
    const instance = render(_jsx(React.StrictMode, { children: _jsx(SettingsContext.Provider, { value: settings, children: _jsx(AppWrapper, { config: config, settings: settings, startupWarnings: startupWarnings, version: version }) }) }), { exitOnCtrlC: false, isScreenReaderEnabled: config.getScreenReader() });
    checkForUpdates()
        .then((info) => {
...
export async function main() {
    ...
    if (config.isInteractive() && mcpServersCount > 0) {
        spinnerInstance = render(_jsx(InitializingComponent, { initialTotal: mcpServersCount }));
    }
