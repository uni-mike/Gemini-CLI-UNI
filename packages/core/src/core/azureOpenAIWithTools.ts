...
export class AzureOpenAIWithTools {
   ...
   */
  private convertToolToFunction(toolName: string): ChatCompletionTool | null {
    const tool = this.toolRegistry.getTool(toolName);
    if (!tool) return null;

    // Map common Gemini tools to OpenAI function schemas
    const functionSchemas: Record<string, ChatCompletionTool> = {
      'read-file': {
        type: 'function',
        function: {
          name: 'read_file',
   ...
   */
  private async executeToolCall(functionName: string, args: any): Promise<string> {
    try {
   ...
   */
  private async executeToolDirectly(toolName: string, params: any): Promise<string> {
    try {
   ...
   */
  private convertFunctionArgsToToolParams(functionName: string, args: any): any {
    // Map function arguments to tool parameters
    const paramMap: Record<string, (args: any) => any> = {
      'read_file': (args) => ({
