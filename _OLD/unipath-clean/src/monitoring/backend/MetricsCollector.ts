  ...
  private async initializeIds() {
    try {
      ...
      if (fs.existsSync(dbFile)) {
        ...
        const query = `INSERT INTO Session (id, projectId, mode, startedAt, turnCount, tokensUsed, status) 
                       VALUES ('${this.sessionId}', '${this.projectId}', 'monitoring', datetime('now'), 0, 0, 'active')`;
        try {
          execSync(`sqlite3 "${dbFile}" "${query}"`, { encoding: 'utf8' });
          console.log('📊 MetricsCollector initialized with project:', this.projectId, 'session:', this.sessionId);
        } catch (err) {
