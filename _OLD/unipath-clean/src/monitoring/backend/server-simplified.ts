...
export class MonitoringServer {
  ...
  private setupRoutes() {
    ...
    this.app.get('/api/events', (req, res) => {
      ...
      // Return empty array when no real events exist
      res.json([]);
    });
    ...
    this.app.get('/api/projects', async (req, res) => {
      try {
        ...
        // Check for .flexicli directory - use relative path to project root
        const flexicliPath = path.join(__dirname, '../../../../.flexicli');
    ...
    this.app.get('/api/sessions', async (req, res) => {
      try {
        // Return empty array when no real sessions exist
        const sessions: any[] = [];
        res.json(sessions);
      } catch (error: any) {
        res.status(500).json({ error: error.message });
